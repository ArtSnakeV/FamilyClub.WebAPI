...
const isOnline = (user: UserInfo) => {
    if (!user.lastLoginAt) return false;
    return Date.now() - new Date(user.lastLoginAt).getTime() < 15 * 60 * 1000;
};
...
export default function Page() {
    ...
    const blockedCount = localUsers.filter(isBlocked).length;
    const activeCount = localUsers.filter((u) => !isBlocked(u) && isOnline(u)).length;

    const { stats, loading } = useUsersStats({
        totalOverride: managerTotal,
        blockedOverride: blockedCount,
        activeOverride: activeCount,
        listLoading: loadingUsersInfo,
    });
