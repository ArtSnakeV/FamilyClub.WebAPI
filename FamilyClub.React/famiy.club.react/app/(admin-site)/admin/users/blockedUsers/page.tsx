...
export default function Page() {
    ...
    const [selectedUserId, setSelectedUserIdBlocked] = useState<string | null>(
        null
    );
    ...
    const temporaryBlocked = blockedUsers.filter(
        (u) => !isPermanentBlock(u.lockoutEnd)
    );
    const permanentBlocked = blockedUsers.filter((u) =>
        isPermanentBlock(u.lockoutEnd)
    );
    ...
    const tabs = [
        {
            key: "allBlocked" as TabKey,
            label: "Всі заблоковані",
            count: blockedUsers.length,
        },
        {
            key: "temporary" as TabKey,
            label: "Тимчасово заблоковані",
            count: temporaryBlocked.length,
        },
        {
            key: "permanent" as TabKey,
            label: "Заблоковані назавжди",
            count: permanentBlocked.length,
        },
        {
            key: "expiring" as TabKey,
        ...
        <div className="w-full min-h-screen overflow-hidden relative m-0 p-0 text-[var(--foreground-primary)]">
            <div className="w-[100vw] min-h-screen relative">
                ...
                <button
                    ...
                    className="absolute top-1 left-6 z-20 w-10 h-10 flex items-center justify-center rounded-full bg-[var(--background-elevated)] hover:bg-[color-mix(in_srgb,var(--color-green)_22%,var(--background-elevated))] transition"
                    aria-label="Назад"
                >
                    <img
