...
import useEditFormatForm from "./hooks/useEditFormatForm";
import useSubmitEditFormat from "./hooks/useSubmitEditFormat";
import { BasicInfoSectionEditFormat } from "./section/BasicInfoSectionEditFormat";
import ButtonReturn from "./ButtonReturn";

export default function EditFormatClient({ id }: { id: string }) {
  const router = useRouter();
  const formatId = Number(id);

  const { form, setField, loading: formLoading } = useEditFormatForm(formatId);

  const { handleSubmit, loading, handleDelete } = useSubmitEditFormat({
    id: formatId,
    form,
    router,
  });
