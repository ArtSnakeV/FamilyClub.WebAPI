
type Props = {
  title: string;
  backgroundImage: string;
  children: React.ReactNode;
  className?: string;
};

export function SectionCard({
  title,
  backgroundImage,
  children,
  className,
}: Props) {
  return (
    <div
      className={`w-full min-h-full ${className ?? ""}`}
      style={{
        backgroundImage: `url('${backgroundImage}')`,
          backgroundSize: "96% 90%",
        backgroundRepeat: "no-repeat",
      }}
    >
      <div
        className="-ml-[4px] relative top-[38px] bg-cover bg-center w-[330px] h-[56px] flex items-center justify-center"
        style={{
          backgroundImage: "url('/images/addProducts/Rectangle 302.svg')",
        }}
      >
        <p className="text-[var(--color-white)] font-['Roboto_Mono'] font-semibold text-[22px] leading-[150%] tracking-[-0.011em] pb-[10px]">
          {title}
        </p>
      </div>
      <div className="w-[480px] ml-[38px] flex flex-col gap-2">{children}</div>
    </div>
  );
}
