...
export default function HistoryPaws({ userId }: { userId?: string }) {
    ...
    const { history, loading } = usePaws(user?.id);
    const {
        currentPage,
        totalPages,
    ...
    return (
        <div className="min-h-screen w-[1000px] flex flex-col ml-[24vw] px-10 py-8" style={{ backgroundColor: "#C7A381" }}>
            ...
            <div className="flex items-center gap-4 mt-36 mb-8 relative z-10">
                <Link href="/paws" className="w-10 h-10 cursor-pointer flex items-center justify-center">
                    <ButtonReturn />
                </Link>
                <h2 className="text-[48px] font-bold text-[var(--color-black)] px-4">Історія лапок</h2>
            </div>
            <div
                className="w-[1110px] -mt-12 -ml-24 h-[800px] relative flex flex-col text-[var(--color-black)]"
            >
                <Image
                ...
                <div className="relative w-full flex-1 min-h-0 flex flex-col items-center text-left justify-center p-2">
                    <p className="text-[24px] -mt-110 font-semibold text-[var(--color-black)] mb-6">
                        В історії зберігаються дії за останні 3 місяці
                    </p>

                    {/* Список */}
                    <div className="flex flex-col space-y-4 ml-6 w-[82%] mt-4 overflow-y-auto max-h-[600px] pr-4">
                        {loading ? (
                            <div className="text-center py-8 text-gray-600">Завантаження історії...</div>
                        ) : paginatedPawsHistory.length === 0 ? (
                            <div className="text-center py-8 text-gray-600">Історія дій поки порожня</div>
                        ) : (
                                    ...
                                    >
                                        <div className="flex w-full flex-row items-center justify-between gap-4">
                                            ...
                                            <div className="flex items-center gap-2 font-bold text-[20px]">
                                                ...
                                                <Image src="/images/pawsUser/Лапка.png" width={34} height={34} alt="paws" />
                                            </div>
                                        </div>
