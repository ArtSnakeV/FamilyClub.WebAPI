...
const cache = new Map<string, { data: MeestCity[]; expiry: number }>();
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const cities: MeestCity[] = addresses
      .filter((a) => a.Present)
      .map((a) => ({
        name: a.Present,
    ...
    console.warn("Meest cities fallback search", error);
